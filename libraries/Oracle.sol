...
library Oracle {
    ...
    function initialize(Observation[65535] storage self) internal returns (uint16 cardinality, uint16 cardinalityNext) {
        self[0] = Observation({blockTimestamp: uint32(block.timestamp), tickCumulative: 0, secondsPerLiquidityCumulativeX128: 0, initialized: true});
        return (1, 1);
    }
    ...
    function write(Observation[65535] storage self, uint16 index, int24 tick, uint128 liquidity, uint16 cardinality, uint16 cardinalityNext) internal returns (uint16 indexUpdated, uint16 cardinalityUpdated) {
        unchecked {
    ...
    function getSurroundingObservations(Observation[65535] storage self, uint32 target, int24 tick, uint16 index, uint128 liquidity, uint16 cardinality) private view returns (Observation memory beforeOrAt, Observation memory atOrAfter) {
        unchecked {
            ...
            if (!lte(uint32(block.timestamp), beforeOrAt.blockTimestamp, target)) revert OLD();
    ...
    function observeSingle(Observation[65535] storage self, uint32 secondsAgo, int24 tick, uint16 index, uint128 liquidity, uint16 cardinality) internal view returns (int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128) {
        unchecked {
            if (secondsAgo == 0) {
                Observation memory last = self[index];
            ...
            (Observation memory beforeOrAt, Observation memory atOrAfter) = getSurroundingObservations(self, target, tick, index, liquidity, cardinality);
    ...
    function observe(Observation[65535] storage self, uint32[] memory secondsAgos, int24 tick, uint16 index, uint128 liquidity, uint16 cardinality) internal view returns (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) {
        unchecked {
            ...
            for (uint256 i = 0; i < secondsAgos.length; i++) {
                (tickCumulatives[i], secondsPerLiquidityCumulativeX128s[i]) = observeSingle(self, secondsAgos[i], tick, index, liquidity, cardinality);
            }
        }
    }
}
